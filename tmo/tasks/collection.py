...
#: Срок аренды шага закрытия. Два часа против пятнадцати минут у остальных
#: шагов: расчёт полной матрицы измерен в 28–37 минут, и аренда обязана его
#: пережить. Меньше жёсткого лимита самой задачи (три часа) — умерший шаг
#: всё равно освободит аренду раньше, чем она превратится в вечную блокировку.
CLOSE_LEASE_TTL = 2 * 3600
...
def close_snapshot(self, snapshot_id: int | None = None,
    ...
    # некому: heartbeat вставить негде, а разбивать расчёт на части ради
    # аренды значит подчинить методику механике блокировки.
    #
    # Прежний срок в 15 минут истекал на середине, диспетчер видел аренду
    # свободной и запускал закрытие заново. Снимок 09.08.2026 получил так пять
    # расчётов подряд, два одновременно; каждый новый заход сбрасывал статус в
    # CALCULATING и начинал с нуля, и снимок не закрывался вовсе.
    with lease.acquire(f"close:{snapshot_id}", ttl=CLOSE_LEASE_TTL) as held:
        if held is None:
