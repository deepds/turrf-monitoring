...
#: Верхняя граница размера пачки: отказ одной пачки не должен уносить слишком
#: много работы.
MAX_BATCH_SIZE = 400

#: Нижняя граница — одно наблюдение, и это не описка.
#:
#: Здесь стояло 20, чтобы накладные расходы транзакций не съели выигрыш на
#: дешёвых семействах. На дорогих граница работала против себя: в ночь
#: 09.08.2026 авианаблюдение стоило 72 секунды при одновременности 2, то есть в
#: бюджет пачки помещалось пять наблюдений, — а граница требовала двадцати.
#: Пачке нужно было 720 секунд при доступных 240, и две трети её уходили в
#: ``BUDGET_EXHAUSTED`` **до всякого обращения к источнику**: 83 потерянных
#: наблюдения из 286 за первый час.
#:
#: Накладные расходы транзакции — это миллисекунды. Потерянные две трети пачки
#: — это часы сбора. Граница снизу не нужна вовсе: на дешёвых семействах бюджет
#: и так даёт сотни наблюдений, а на дорогих любое зажатие снизу означает
#: заведомо невыполнимую пачку.
MIN_BATCH_SIZE = 1
